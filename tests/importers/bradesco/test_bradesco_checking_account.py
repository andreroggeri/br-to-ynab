...
class TestBradescoCheckingAccount(unittest.TestCase):

    def test_should_get_transactions(self):
        fake_bradesco = Mock(Bradesco)
        fake_bradesco.get_checking_account_statements.return_value = fake_transactions()

        importer = BradescoCheckingAccount(fake_bradesco, '1234')
